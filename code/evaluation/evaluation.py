...
def preload_graph_embeddings(embeder, graph, batch_size=64, save_cache=True):
    ...
    A single warmup traversal only initializes the model on a small set of nodes.
    Without this preloading step, the first evaluated dimension for each model
    pays for many first-time SentenceTransformer.encode() calls inside the timed
    traversal loop. Later Matryoshka dimensions then look much faster because
    they reuse the embeddings computed by the full-dimension run.
    """
