...
def test_ablation_model_names_include_reference_and_three_variants():
    assert utils.get_ablation_model_names("unit") == [
        "granite-embedding-english-r2_relu_euclid_nonorm_matryoshka_unit_finetuned",
        "granite-embedding-english-r2_relu_cosine_nonorm_matryoshka_unit_ablation_finetuned",
        "granite-embedding-english-r2_gelu_euclid_nonorm_matryoshka_unit_ablation_finetuned",
        "granite-embedding-english-r2_gelu_cosine_nonorm_matryoshka_unit_ablation_finetuned",
    ]


def test_ablation_model_paths_keep_the_comparison_order(tmp_path, monkeypatch):
    expected_names = utils.get_ablation_model_names("unit")
    ...
    model_paths = utils.get_ablation_fine_tuned_models("unit")
